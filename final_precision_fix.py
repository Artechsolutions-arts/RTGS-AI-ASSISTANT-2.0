...
const dateIST = new Date(new Date(nowMs).toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
...
if ((intent === 'request_appointment' || isBookingKeyword) && intent !== 'view_calendar') {
    ...
    if (ents.time && ents.time.length > 0) {
        ...
        if (match) {
            ...
            if (ampm === 'am' && h === 12) h = 0;
            reqStart = `${y}-${m}-${d}T${pad(h)}:${pad(min)}:00+05:30`;
            const endH = (min === 0) ? h + 1 : h;
            const endM = (min === 0) ? 0 : (min + 1) % 60;
...
if (dateQuery.intent === 'request_appointment') {
    ...
    const collisions = uniqueEvents.filter(e => {
        const s = new Date(e.json.start.dateTime).getTime();
        const en = new Date(e.json.end.dateTime).getTime();
...
if (dateQuery.intent === 'view_calendar') {
    ...
    if (dateQuery.dateLabel === 'Today') {
        filtered = uniqueEvents.filter(e => new Date(e.json.end.dateTime).getTime() > (nowMs - 60000));
    }
    if (filtered.length === 0) {
        return [{ json: { chat_id: parseInt(userChatId), message: '📅 *Schedule for Today*\n\nAll meetings have concluded! ✅', bot_token: botToken } }];
    }
    let msg = '📅 *Schedule for ' + dateQuery.dateLabel + '*\n\n';
    filtered.forEach((e, i) => {
        const start = new Date(e.json.start.dateTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Asia/Kolkata' });
        const end = new Date(e.json.end.dateTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Asia/Kolkata' });
        msg += (i + 1) + '. *' + (e.json.summary || 'Meeting') + '*\n   🕐 ' + start + ' - ' + end + '\n\n';
    });
