...
for node in nodes:
    # 1. Update Format Calendar Response to clean the Reason field and improve formatting
...
if (dateQuery.intent === 'request_appointment') {
    ...
    // Time extraction from text (Case-Insensitive)
    if (!reqStart) {
    ...
    if (!reqStart) {
        return [{ json: { chat_id: parseInt(userChatId), message: '🕒 *Time missing:* Please specify the time (e.g., "at 2:00 PM tomorrow").', bot_token: botToken, parse_mode: 'Markdown' } }];
    }
    ...
    if (isBusy) {
        ...
        // CLEAN REASON: Remove time and tomorrow/today keywords from the reason field
...
print("Reason field cleaned (time/date keywords removed) and formatting improved.")
