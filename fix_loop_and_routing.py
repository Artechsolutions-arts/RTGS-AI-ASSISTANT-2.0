...
for node in nodes:
    # 1. Update Check Intent Switch Node to catch follow-up messages
    if node.get('id') == "f31c3b6e-7626-4117-a27f-210248d59575": # Check Intent
        node['parameters']['rules']['values'][0]['conditions']['conditions'] = [
            {
            ...
            },
            {
                ...
                "rightValue": "reason:|name:|appointment|meet|schedule|slot",
                "operator": { "type": "string", "operation": "regex" },
                "id": "cal-route-text"
            }
        ]
        node['parameters']['rules']['values'][0]['conditions']['combinator'] = "or"

    # 2. Update Extract Date Query for better parsing
...
return [{
    ...
    # 3. Update Format Calendar Response for consistent 1-hour checks
...
if (dateQuery.intent === 'request_appointment') {
    ...
    // Backup time extraction
    if (!reqStart) {
        const timeMatch = dateQuery.messageText.match(/at\s*(\d{1,2})[:\.]?(\d{2})?\s*(am|pm)/i);
        if (timeMatch) {
    ...
    if (!reqStart) {
        // Only ask for time if this isn't just a follow-up about a reason
        if (dateQuery.applicantName || dateQuery.meetingReason) {
             return [{ json: { chat_id: parseInt(userChatId), message: '🕒 *Time missing:* I have your details, but please tell me what time you would like the meeting (e.g. "at 2pm today").', bot_token: botToken, parse_mode: 'Markdown' } }];
        }
        return [{ json: { chat_id: parseInt(userChatId), message: '❓ *Time Required*\n\nPlease specify a time (e.g. "tomorrow at 2pm").', bot_token: botToken, parse_mode: 'Markdown' } }];
    }

    const reqS = new Date(reqStart).getTime();
    const reqE = reqS + 3600000; // Book for 1 hour
    ...
    if (isBusy) {
        ...
        for (let h = 10; h < 17; h++) {
            ...
            for (let m of ['00', '30']) {
                ...
                const scanE = scanS + 3600000; // Check for a full 1h availability
        ...
        if (!finalReason) {
             return [{ json: { chat_id: parseInt(userChatId), message: `✅ *${specificTimeLabel}* is available!\n\nPlease provide the **Reason** and your **Full Name** (e.g. "Reason: Discussion, Name: Revanth").`, bot_token: botToken, parse_mode: 'Markdown' } }];
        }
        ...
        return [
            { json: { chat_id: parseInt(userChatId), message: `✅ *Request Sent*\n\nSent to Collector: *${specificTimeLabel}* (Reason: ${finalReason})`, bot_token: botToken, parse_mode: 'Markdown' } },
...
print("Busy loop fixed (matching durations) and follow-up routing enabled.")
