...
for node in nodes:
    # 1. Update Extract Date Query for better parsing and intent stickiness
...
let dateLabel = 'Today';
if (messageText.includes('tomorrow') || messageText.includes('రేపు') || messageText.includes('next day')) {
...
return [{
    ...
    # 2. Update Format Calendar Response for Next-Day Scanning and Context Handling
...
if (dateQuery.intent === 'request_appointment') {
    ...
    // Backup time extraction from text
    if (!reqStart) {
    ...
    // SCENARIO: Time is missing (User sent Reason/Name only)
    if (!reqStart) {
        return [{ json: { chat_id: parseInt(userChatId), message: '🕒 *Time missing:* I have your details, but please confirm the date/time one last time (e.g., "at 2 PM tomorrow").', bot_token: botToken, parse_mode: 'Markdown' } }];
    }
    ...
    if (isBusy) {
        // SCENARIO: Slot Busy -> Scan for Suggestions
        let suggestion = '';
        let sugDateLabel = dateQuery.dateLabel;
        
        // Helper function for scanning a specific day
        const scanDay = (baseDateStr, isNextDay) => {
            for (let h = 10; h < 17; h++) {
        ...
        suggestion = scanDay(dateQuery.targetDate, false);
        
        // If today is full, try tomorrow!
        if (!suggestion) {
            ...
            suggestion = scanDay(tomStr, true);
        ...
        // SCENARIO: Slot Available -> Check for details
        const finalName = dateQuery.applicantName || userName;
        const finalReason = dateQuery.meetingReason;

        if (!finalReason) {
             return [{ json: { chat_id: parseInt(userChatId), message: `✅ *${specificTimeLabel}* is available!\n\nPlease send your **Reason and Full Name** and mention the time again to confirm (e.g. "Reason: Discussion, Name: Revanth at ${specificTimeLabel}").`, bot_token: botToken, parse_mode: 'Markdown' } }];
        }
        ...
        return [
            { json: { chat_id: parseInt(userChatId), message: `✅ *Request Sent*\n\nMeeting for *${specificTimeLabel}* has been sent for approval.`, bot_token: botToken, parse_mode: 'Markdown' } },
...
print("Next-day suggestion scan and context handling enabled.")
